...
for date in $date_data; do
    ...
    if [ $? -eq 0 ]; then
        echo "Successfully committed for ${commit_date}. Removing date from ${date_file}."
        ...
        echo "Commit failed for ${commit_date}. Date will remain in ${date_file}."
        # Exit or break the loop if commit fails to prevent further processing
